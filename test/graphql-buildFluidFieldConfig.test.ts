...
test("returns a namespaced GraphQL field config", (t) => {
	...
	t.like(actual, {
		type: "NamespaceFluid",
		args: {
			maxWidth: { type: "Int", defaultValue: 800 },
			maxHeight: { type: "Int" },
			srcSetBreakpoints: { type: "[Int!]" },
...
test("resolves fluid object using generateImageSource", async (t) => {
	...
	const fieldConfig = libServer.buildFluidFieldConfig({
		namespace: "Namespace",
		generateImageSource,
	});
...
test("includes default imgix params in fluid object", async (t) => {
	const fieldConfig = libServer.buildFluidFieldConfig({
		namespace: "Namespace",
		generateImageSource: () => ({
			url: "https://example.com/image.png?foo=bar",
...
test("includes args imgix params in fluid object", async (t) => {
	const fieldConfig = libServer.buildFluidFieldConfig({
		namespace: "Namespace",
		generateImageSource: () => ({
			url: "https://example.com/image.png?foo=bar",
